/*
 * TODO License + reference to original source
 */

/**
 * @properties={typeid:24,uuid:"796787EE-43AF-4261-8577-CF019F1691B8"}
 */
function CSSParser() {
...
/**
 * @properties={typeid:35,uuid:"F929ACC2-FA2D-4BA1-9742-BE44E79A1DCB",variableType:-4}
 */
var initFi = (function() {
		/*
		 ...
		 */
		 CSSParser.prototype.stripComments = function(cssString) {
		...
		/*
		 ...
		 @return object css
		 */
		CSSParser.prototype.parseCSS = function(source) {
		...
		/*
		 ...
		 \n\ncolor:white;\n    font-size:18px;\n
		 */
		CSSParser.prototype.parseRules = function(rules) {
		 ...
		 */
		CSSParser.prototype.findCorrespondingRule = function(rules, directive, value) {
			if (value === undefined) {
				value = false;
			}
			var ret = false;
		...
		/*
		 ...
		 and returns them
		 */
		CSSParser.prototype.findBySelector = function(cssObjectArray, selector, contains) {
		 ...
		 */
		CSSParser.prototype.deleteBySelector = function(cssObjectArray, selector) {
			var ret = [];
			for (var i = 0; i < cssObjectArray.length; i++) {
				if (cssObjectArray[i].selector !== selector) {
		...
		/*
		 ...
		 selector redundence.
		 */
		CSSParser.prototype.compressCSS = function(cssObjectArray) {
			var compressed = [];
			var done = { };
			for (var i = 0; i < cssObjectArray.length; i++) {
		 ...
		 */
		CSSParser.prototype.cssDiff = function(css1, css2) {
		 ...
		 */
		CSSParser.prototype.intelligentMerge = function(cssObjectArray, newArray, reverse) {
		 ...
		 */
		CSSParser.prototype.intelligentCSSPush = function(cssObjectArray, minimalObject, reverse) {
		 ...
		 */
		CSSParser.prototype.compactRules = function(rules) {
			var newRules = [];
			for (var i = 0; i < rules.length; i++) {
		 ...
		 */
		CSSParser.prototype.getCSSForEditor = function(cssBase, depth) {
			if (depth === undefined) {
				depth = 0;
			}
			var ret = '';
		...
		CSSParser.prototype.getImports = function(cssObjectArray) {
			var imps = [];
			for (var i = 0; i < cssObjectArray.length; i++) {
		 ...
		 */
		CSSParser.prototype.getCSSOfRules = function(rules, depth) {
			var ret = '';
			for (var i = 0; i < rules.length; i++) {
		 ...
		 */
		CSSParser.prototype.getSpaces = function(num) {
			var ret = '';
		 ...
		 */
		CSSParser.prototype.applyNamespacing = function(css, forcedNamespace) {
		 ...
		 */
		CSSParser.prototype.clearNamespacing = function(css, returnObj) {
